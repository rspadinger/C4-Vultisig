...
describe("Whitelist", function () {
    ...
    describe("Ownable", function () {
        ...
        it("Should revert if called from non-owner address", async function () {
            ...
            await expect(whitelist.connect(otherAccount).setLocked(true)).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(whitelist.connect(otherAccount).setMaxAddressCap(10_000)).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(whitelist.connect(otherAccount).setVultisig(mockContract)).to.be.revertedWith("Ownable: caller is not the owner")
            await expect(whitelist.connect(otherAccount).setIsSelfWhitelistDisabled(true)).to.be.revertedWith(
                "Ownable: caller is not the owner",
            )
            await expect(whitelist.connect(otherAccount).setOracle(mockContract)).to.be.revertedWith("Ownable: caller is not the owner")
            ...
            await expect(whitelist.connect(otherAccount).addBatchWhitelist([otherAccount, batchedAccount])).to.be.revertedWith(
                "Ownable: caller is not the owner",
            )
        })
    })
    ...
    describe("Checkwhitelist", function () {
        ...
        it("Should revert when called from non vultisig contract", async function () {
            ...
            await expect(whitelist.checkWhitelist(pool, otherAccount, 0)).to.be.revertedWithCustomError(whitelist, "NotVultisig")
        })
