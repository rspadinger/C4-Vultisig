...
abstract contract IntegrationTestBase is Mock, Test {
    ...
    function _setupBase() internal {
        //@note create a mainnet fork
        uint256 mainnetFork = vm.createFork("https://rpc.ankr.com/eth", 20024256);
        vm.selectFork(mainnetFork);

        //@note start a broadcast of txn's to mainnet fork => using the address that calls the test contract has all subsequent calls
        //create transactions that can later be signed and sent onchain.
        vm.startBroadcast(MANAGER_OWNER);
        //@note add salt to contract creation
        ...
        //@note Sets up a prank from an address that has some ether
        hoax(PROJECT_OWNER);
    ...
    function _writeTokenBalance(address token, address who, uint256 amt) internal {
        //@note write to specific storage slot
