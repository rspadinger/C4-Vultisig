...
contract ILOManager is IILOManager, Ownable, Initializable {
    ...
    // @inheritdoc IILOManager => One project can init many ILO Pool
    function initILOPool(
        InitPoolParams calldata params
    ) external override onlyProjectAdmin(params.uniV3Pool) returns (address iloPoolAddress) {
        ...
        //@note create & deploy a clone of the pool contract => this allows us to know the contract address immediately
    ...
    function _initUniV3PoolIfNecessary(PoolAddress.PoolKey memory poolKey, uint160 sqrtPriceX96) internal returns (address pool) {
        ...
        if (pool == address(0)) {
            ...
            //@audit-ok add additionl function to contract to set the correct sqrtPriceX96
            ...
            if (sqrtPriceX96Existing == 0) {
                ...
                //@audit-ok - are those prices really exactly the same
    ...
    function _cacheProject(
        ...
        _project.admin = msg.sender; // th one who calls initProject
    ...
    // project launch :: we launch all ILOPools => call launch on ILOPool
    function launch(address uniV3PoolAddress) external override {
        ...
        //@audit-ok slot0 can be manipulated => set in initialization
    ...
    function claimRefund(
        address uniV3PoolAddress
    ) external override onlyProjectAdmin(uniV3PoolAddress) returns (uint256 totalRefundAmount) {
        //@audit-ok > block.ts
